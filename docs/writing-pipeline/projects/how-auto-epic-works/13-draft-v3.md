...
Nine hook scripts intercept tool calls at three lifecycle points: PreToolUse (blocks before action), PostToolUse (auto-corrects after action), and Stop (validates quality gates at completion). A tenth hook, the Stop hook, is agent prompt-based rather than script-based (`.claude/hooks/README.md:8-20`).
...
<!-- Word count: 2,036 words | Target: 2,400 words | Δ: -364 (-15%) -->
