...
const IMPL_FILE_PATTERNS = [/\.[jt]sx?$/];
...
process.stdin.on("end", () => {
  ...
  try {
    ...
    // Fail CLOSED on parse error — do not allow unvalidated writes
    console.error(
      `🧪 tdd-guard: Failed to parse input (${err.message}). Blocking write for safety.`
    );
    process.exit(2);
  }
});
